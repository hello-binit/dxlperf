...
class TransportSHM {
    private:
         ...
         float x;
         int transactions;
         void * shm_push_stream;
         void * shm_push_one_shot;
         void * shm_reply;
         bool shm_ready;
         Serial * ser;

         uint8_t frame_buf_out[RPC_MAX_FRAME_SIZE];
...
arr.array('B', [0] * RPC_MAX_FRAME_SIZE)
    ...
    inline bool do_push_transaction(char * payload, int nb_payload)
    {
        /*
        Parameters
        ----------
        payload: Buffer of data to transmit in RPC to uC
        nb_payload: number of bytes in payload
...
*/
        ...
        int n_frames = std::ceil(nb_payload / RPC_V1_FRAME_DATA_MAX_BYTES);
        int widx = 0
        std::memset(frame_buf_out,0); 
